...
function DENodesAndWeights{T<:Number}(z::Array{Complex{T},1},gamma::T,n::Integer;digits::Integer=77,domain::Domain=Finite(zero(T),zero(T),zero(T),zero(T)),Hint::Integer=10,obj_scaling_factor::Float64=-1.0)
	#
	# On entry:
	#
	...
	u0,u,xpre = DEMapValues(z,gamma;digits=digits,domain=domain,Hint=Hint,obj_scaling_factor=obj_scaling_factor)
	
	b2opt = u0
	dDEopt = convert(T,pi)/2/gamma
	gaopt = gamma
...
function DENodesAndWeights{T<:Number}(u0::T,gamma::T,u::Array{T,1},n::Integer;digits::Integer=77,domain::Domain=Finite(zero(T),zero(T),zero(T),zero(T)),Hint::Integer=10,obj_scaling_factor::Float64=-1.0)
	#
	# On entry:
	#
...
function DEMapValues{T<:Number}(z::Array{Complex{T},1},gamma::Real{T};digits::Integer=77,domain::Domain=Finite(zero(T),zero(T),zero(T),zero(T)),Hint::Integer=10,obj_scaling_factor::Float64=-1.0)
	#
	# On entry:
	#
...
function eval_f(x)
  ...
  for k=1:n
	temp3=0.0
	for j=1:n
		temp3+=x[n+j]*complex(x[k],pi/(2*gamma))^(j-1)
	end
...
function eval_g(x, g)
  ...
  for k=1:n
	temp1=0.0
	for j=1:n
		temp1+=x[n+j]*complex(x[k],pi/(2*gamma))^(j-1)
	end
...
function eval_grad_f(x, grad_f)
  ...
  for r=1:n
	...
	for k=1:n
		...
		temp1+=ept[k]-imag(temp3)
		temp2+=cosh(x[k])
		temp4+=x[n+k]*(k-1)*complex(x[r],pi/(2*gamma))^(k-2)
  ...
  if mode == :Structure
	...
	for k=1:n
		...
		for r=1:n
			...
			temp1+=x[n+r]*(r-1)*complex(x[k],pi/(2*gamma))^(r-2)
			values[2n*(k-1)+n+r] = real(complex(x[k],pi/(2*gamma))^(r-1))
			
			temp2+=x[n+r]*complex(x[k],pi/(2*gamma))^(r-1)
			
			temp3=0.0
			for j=1:n
				temp3+=x[n+j]*complex(x[k],pi/(2*gamma))^(j-1)
			end
			values[2n^2+2n*(k-1)+n+r] += imag(complex(x[k],pi/(2*gamma))^(r-1))/cosh(x[k])
		end
